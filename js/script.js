...
let spikes = [];
let spikes2 = []; // Mảng riêng cho SparkPoint2
function init(a) {
  positions = [];
  colors = [];
...
function render(a) {
  ...
  spikes2.forEach((g2, i) => {
    g2.update(a);
    const rand = g2.rand;
    const color = g2.color;
    // Thay đổi kích thước và vị trí của hình thứ hai
    if (maxZ * rateZ + rand > g2.one.z && g2.one.z > -maxZ * rateZ - rand) {
      positions.push(g2.one.x * 2, g2.one.y * 2, g2.one.z * 2); // Thay đổi kích thước
      colors.push(color.r, color.g, color.b);
    }
    if (
      maxZ * rateZ + rand * 2 > g2.one.z &&
      g2.one.z > -maxZ * rateZ - rand * 2
    ) {
      positions.push(g2.two.x, g2.two.y, g2.two.z);
      colors.push(color.r, color.g, color.b);
    }
  });
...
const palette2 = [
  new THREE.Color("#ffd4ee"), // Màu hồng nhạt
  new THREE.Color("#ff77fc"), // Màu hồng đậm
  new THREE.Color("#ff77ae"), // Màu hồng đậm hơn
  new THREE.Color("#ff1775"), // Màu đỏ đậm
  new THREE.Color("#ff0055"), // Màu đỏ
  new THREE.Color("#ff0000"), // Màu đỏ sáng
];
// Hàm để thay đổi màu sắc của SparkPoint
// function changeColors() {
//   spikes.forEach((g) => {
    
//     g.color = newColors[Math.floor(Math.random() * newColors.length)];
//   });
// }
...
function changeColors() {
  if (currentIndex % 3 === 0) {
    ...
    spikes.forEach((g) => {
      g.color = palette2[Math.floor(Math.random() * palette2.length)];
    });
  }
  else{
    spikes.forEach((g) => {
...
// Sử dụng setInterval để gọi hàm thay đổi màu sắc mỗi giây
setInterval(changeColors, 1000); // 1000 milliseconds = 1 giây
